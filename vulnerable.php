...
<h2>🔍 商品検索（脆弱）</h2>
<form action="/vulnerable.php?page=search" method="get">
...
<h2>🔐 ログイン（脆弱）</h2>
<form action="/vulnerable.php?page=login" method="post">
...
function pageSearch(PDO $pdo): void {
    ...
    if ($q === '') {
        layout('検索', '<p>検索キーワードを入力してください。</p><p><a href="/vulnerable.php">← 戻る</a></p>');
        return;
    }
    ...
    // =============================================
        ...
        $body = '<h1>検索結果（' . $count . '件）</h1>'
              . $queryDisplay
              . $items
              . '<p><a href="/vulnerable.php">← 戻る</a></p>';
        layout('検索結果', $body);
    } catch (Exception $e) {
...
function pageLogin(PDO $pdo): void {
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'POST') {
        ...
        layout('Method Not Allowed', '<p>POST メソッドを使用してください。</p><p><a href="/vulnerable.php">← 戻る</a></p>');
        return;
    }
    ...
    if ($username === '') {
        layout('ログイン', '<p>ユーザー名を入力してください。</p><p><a href="/vulnerable.php">← 戻る</a></p>');
        return;
    }
...
try {
    ...
    layout('エラー', '<div class="warning">' . h($e->getMessage()) . '</div><p><a href="/vulnerable.php">← 戻る</a></p>');
}